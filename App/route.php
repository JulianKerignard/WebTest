 ...
 *
 * Ce fichier contient toutes les routes de l'application organisées par catégories.
 * Il est chargé par l'application principale pour configurer le routeur.
 */
...
function registerRoutes(App $app) {
    // Récupérer les instances du routeur et des middlewares
    $router = $app->router;
    ...
    $rateLimit = new \App\Middleware\RateLimiterMiddleware(5, 10);
    ...
    $router->post('/login', [\App\Controllers\AuthController::class, 'authenticate'], [$csrfMiddleware, $rateLimit]);
    ...
    $router->post('/forgot-password', [\App\Controllers\AuthController::class, 'sendResetLink'], [$csrfMiddleware, $rateLimit]);
    ...
    $router->post('/profile/update', [\App\Controllers\UserController::class, 'updateProfile'], [$authMiddleware, $csrfMiddleware]);
    $router->post('/profile/change-password', [\App\Controllers\UserController::class, 'changePassword'], [$authMiddleware, $csrfMiddleware]);
    ...
    $router->post('/company/rate', [\App\Controllers\CompanyController::class, 'rate'], [$studentMiddleware, $csrfMiddleware]);
    ...
    $router->get('/pilot/companies', [\App\Controllers\PilotController::class, 'companies'], [$adminPilotMiddleware]);
    ...
    $router->get('/admin/companies/edit/{id}', [\App\Controllers\CompanyController::class, 'edit'], [$adminMiddleware]);
    $router->post('/admin/companies/update/{id}', [\App\Controllers\CompanyController::class, 'update'], [$adminMiddleware, $csrfMiddleware]);
    ...
    $router->get('/admin/internships/edit/{id}', [\App\Controllers\InternshipController::class, 'edit'], [$adminMiddleware]);
    $router->post('/admin/internships/update/{id}', [\App\Controllers\InternshipController::class, 'update'], [$adminMiddleware, $csrfMiddleware]);
    $router->post('/admin/internships/delete/{id}', [\App\Controllers\InternshipController::class, 'delete'], [$adminMiddleware, $csrfMiddleware]);

    // --------------------------------
    // Gestion des candidatures (Admin et Pilote)
    // --------------------------------

    $router->post('/applications/update-status', [\App\Controllers\ApplicationController::class, 'updateStatus'], [$adminPilotMiddleware, $csrfMiddleware]);
    $router->post('/applications/add-note', [\App\Controllers\ApplicationController::class, 'addNote'], [$adminPilotMiddleware, $csrfMiddleware]);
