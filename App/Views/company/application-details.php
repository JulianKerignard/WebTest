...
<div class="page-title">
    ...
    <p>Candidature de <strong><?= View::escape_value($application, 'student_name', 'Étudiant') ?></strong> pour le stage <strong><?= View::escape_value($application, 'Offer_title', 'Non spécifié') ?></strong></p>
</div>
...
<div class="application-detail-container">
    <div class="application-header">
        ...
        <div class="application-title">
            <h2><?= View::escape_value($application, 'student_name', 'Étudiant') ?></h2>
            <p class="student-info"><?= View::escape_value($application, 'study_field', 'Non spécifié') ?> - <?= View::escape_value($application, 'school_name', 'Non spécifié') ?></p>
            <p class="application-meta">
                <span class="application-date"><i class="far fa-calendar"></i> Candidature reçue le <?= View::date($application, 'created_at', 'd/m/Y', 'Date inconnue') ?></span>
                <span class="application-email"><i class="far fa-envelope"></i> <?= View::escape_value($application, 'student_email', 'Email non disponible') ?></span>
            </p>
        </div>
        ...
        <div class="application-column">
            <div class="application-card status-management">
                <div class="card-header-alt">
                    ...
                    <span class="status-badge <?= View::safe($application, 'status', '') ?>"><?= View::escape_value($application, 'status_label', 'Non défini') ?></span>
                </div>
                <div class="status-form" id="statusForm" style="display: none;">
                    <form id="updateStatusForm">
                        <input type="hidden" name="application_id" value="<?= View::safe($application, 'id', 0) ?>">
                            ...
                            <select name="status" id="status" class="form-control" required>
                                <?php if (isset($status_options) && is_array($status_options)): ?>
                                    <?php foreach ($status_options as $value => $label): ?>
                                        <option value="<?= $value ?>" <?= View::safe($application, 'status') === $value ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        ...
                        <?php foreach ($application['status_history'] as $history): ?>
                            <div class="timeline-item">
                                ...
                                <div class="timeline-content">
                                    <h4><?= View::escape_value($history, 'status_label', 'Statut inconnu') ?></h4>
                                    <p class="timeline-date"><?= View::escape_value($history, 'time_ago', '') ?></p>
                        ...
                        <?php foreach ($application['notes'] as $note): ?>
                            <div class="note-item">
                                <div class="note-header">
                                    <span class="note-author"><?= View::escape_value($note, 'author_name', 'Utilisateur') ?></span>
                                    <span class="note-date"><?= isset($note['created_at']) ? $note['created_at'] : '' ?></span>
                                </div>
                                <div class="note-content">
                                    <?= View::has($note, 'content') ? nl2br(htmlspecialchars($note['content'])) : '' ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <div class="application-card">
                ...
                <div class="internship-details">
                    <div class="detail-group">
                        ...
                        <p><?= View::escape_value($application, 'Offer_title', 'Non spécifié') ?></p>
                    </div>
                    <div class="detail-group">
                        <h4>Durée</h4>
                        <p><?= View::escape_value($application, 'internship_duration', 'Non spécifié') ?></p>
                    </div>
                    <div class="detail-group">
                        <h4>Rémunération</h4>
                        <p><?= View::escape_value($application, 'monthly_remuneration', 'Non spécifié') ?> €/mois</p>
                    </div>
                    <div class="detail-group">
                        <h4>Date de début</h4>
                    ...
                    <div class="detail-group">
                        <h4>Localisation</h4>
                        <p><?= View::escape_value($application, 'location', 'Non spécifié') ?></p>
                    </div>
                </div>
                <div class="card-actions">
                    <a href="/company/offers/<?= View::safe($application, 'offer_id', 0) ?>" class="btn btn-outline">Voir l'offre complète</a>
                </div>
            </div>
        </div>
    </div>
